...
void USBCamera::configure_trigger(USBCamera::TriggerType type)
{
    ...
    if (!IsAvailable(trigger_mode) || !IsReadable(trigger_mode))
        throw std::runtime_error("Unable to disable trigger mode (node retrieval).");
    ...
    if (!IsAvailable(trigger_mode_off) || !IsReadable(trigger_mode_off))
        throw std::runtime_error("Unable to disable trigger mode (enum entry retrieval).");
    ...
    {
        CEnumEntryPtr trigger_source_software = trigger_source->GetEntryByName("Software");
        if (!IsAvailable(trigger_source_software) || !IsReadable(trigger_source_software))
            throw std::runtime_error("Unable to set trigger mode (enum entry retrieval).");

        trigger_source->SetIntValue(trigger_source_software->GetValue());
    }
    else if (type == USBCamera::TriggerType::HARDWARE)
    {
        CEnumEntryPtr trigger_source_hardware = trigger_source->GetEntryByName("Line0");
        if (!IsAvailable(trigger_source_hardware) || !IsReadable(trigger_source_hardware))
            throw std::runtime_error("Unable to set trigger mode (enum entry retrieval).");
    ...
    if (!IsAvailable(trigger_mode_on) || !IsReadable(trigger_mode_on))
        throw std::runtime_error("Unable to enable trigger mode (enum entry retrieval).");
...
void USBCamera::reset_trigger()
{
    ...
    CEnumEntryPtr trigger_mode_off = trigger_mode->GetEntryByName("Off");
    if (!IsAvailable(trigger_mode_off) || !IsReadable(trigger_mode_off))
        throw std::exception("Unable to disable trigger mode (enum entry retrieval).");
...
void USBCamera::print_device_info()
{
    ...
    if (IsAvailable(category) && IsReadable(category))
    {
        ...
        for (it = features.begin(); it != features.end(); ++it)
        {
            ...
            std::cout << (IsReadable(value) ? value->ToString() : "Node not readable");
            std::cout << endl;
        }
    }
    else
...
void USBCamera::set_aquisition_mode(gcstring mode)
{
    ...
    if (!IsAvailable(acquisition_mode) || !IsWritable(acquisition_mode))
        throw std::runtime_error("Unable to set acquisition mode to " + mode + " (node retrieval).");

    CEnumEntryPtr mode_entry = acquisition_mode->GetEntryByName(mode);
    if (!IsAvailable(mode_entry) || !IsReadable(mode_entry))
        throw std::runtime_error("Unable to set acquisition mode to " + mode + " (entry retrieval).");

    acquisition_mode->SetIntValue(mode_entry->GetValue());
}

void USBCamera::begin_acquisition()
{
    _ptr->BeginAcquisition();
}
...
void USBCamera::end_acquisition()
{
    _ptr->EndAcquisition();
}
